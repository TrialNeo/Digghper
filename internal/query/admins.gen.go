...
func newAdmin(db *gorm.DB, opts ...gen.DOOption) admin {
	...
	am.adminDo.UseDB(db, opts...)
	am.adminDo.UseModel(&dao.Admin{})

	tableName := am.adminDo.TableName()
	am.ALL = field.NewAsterisk(tableName)
	am.ID = field.NewUint(tableName, "id")
	am.Username = field.NewString(tableName, "username")
	am.Password = field.NewString(tableName, "password")
	am.LastIp = field.NewString(tableName, "last_ip")
	am.Ips = field.NewString(tableName, "ips")
	am.CreatedAt = field.NewTime(tableName, "created_at")
	am.UpdatedAt = field.NewTime(tableName, "updated_at")
...
func (a *admin) updateTableName(table string) *admin {
	...
	a.Ips = field.NewString(table, "ips")
...
type IAdminDo interface {
	...
	Unscoped() IAdminDo
	Create(values ...*dao.Admin) error
	CreateInBatches(values []*dao.Admin, batchSize int) error
	Save(values ...*dao.Admin) error
	First() (*dao.Admin, error)
	Take() (*dao.Admin, error)
	Last() (*dao.Admin, error)
	Find() ([]*dao.Admin, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*dao.Admin, err error)
	FindInBatches(result *[]*dao.Admin, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*dao.Admin) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*dao.Admin, error)
	FirstOrCreate() (*dao.Admin, error)
	FindByPage(offset int, limit int) (result []*dao.Admin, count int64, err error)
...
func (a adminDo) Create(values ...*dao.Admin) error {
	if len(values) == 0 {
		return nil
	}
...
func (a adminDo) CreateInBatches(values []*dao.Admin, batchSize int) error {
	return a.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (a adminDo) Save(values ...*dao.Admin) error {
	if len(values) == 0 {
		return nil
	}
...
func (a adminDo) First() (*dao.Admin, error) {
	if result, err := a.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*dao.Admin), nil
	}
}

func (a adminDo) Take() (*dao.Admin, error) {
...
func (a adminDo) Last() (*dao.Admin, error) {
...
func (a adminDo) Find() ([]*dao.Admin, error) {
	result, err := a.DO.Find()
	return result.([]*dao.Admin), err
}

func (a adminDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*dao.Admin, err error) {
	buf := make([]*dao.Admin, 0, batchSize)
...
func (a adminDo) FindInBatches(result *[]*dao.Admin, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (a adminDo) FirstOrInit() (*dao.Admin, error) {
...
func (a adminDo) FirstOrCreate() (*dao.Admin, error) {
...
func (a adminDo) FindByPage(offset int, limit int) (result []*dao.Admin, count int64, err error) {
...
func (a adminDo) Delete(models ...*dao.Admin) (result gen.ResultInfo, err error) {
